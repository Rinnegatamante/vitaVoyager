...
static void update_variables(bool startup)
{
	...
	var.key = "vitavoyager_resolution";
	var.value = NULL;
	...
	var.key = "vitavoyager_invert_y_axis";
	var.value = NULL;
...
void retro_get_system_info(struct retro_system_info *info)
{
   ...
   info->library_name     = "vitaVoyager";
#ifndef GIT_VERSION
#define GIT_VERSION ""
#endif
   info->library_version  = "v0.5" ;
...
bool retro_load_game(const struct retro_game_info *info)
{
	...
	{
		if (log_cb)
			log_cb(RETRO_LOG_ERROR, "vitaVoyager: libretro frontend doesn't have OpenGL support.\n");
		return false;
	}
...
static void NET_GetLocalAddress( void ) {
	...
	if(!getaddrinfo_retro(hostname, NULL, &hint, &res))
	{
